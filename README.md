...
- The chatbot connects to a locally hosted model served via a FastAPI endpoint. FastAPI can handle chat completion requests which is compatible with the OpenAI API. The model is loaded into the memory of an NVIDIA A100 80GB GPU for inference, but can also run on a CPU. 
